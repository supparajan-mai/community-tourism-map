...
const App = () => {
  ...
  useEffect(() => {
    ...
    }, 15000); // เช็กทุก 15 วินาทีให้ไวขึ้น
  ...
  return (
    <div className="bg-gray-50 min-h-screen max-w-md mx-auto relative shadow-2xl border-x border-gray-100 font-sans text-gray-900 overflow-hidden">
      ...
      <div className="h-full overflow-y-auto custom-scrollbar pb-32">
        ...
        {/* Tab อื่นๆ ใช้โค้ดเดิมที่พี่ชื่นชอบ */}
        {activeTab === 'memo' && (
        ...
        {activeTab === 'finance' && (
          <div className="p-4 space-y-6 animate-fade-in">
            ...
            <div className="space-y-2">
              {transactions.map(t => (
                <div key={t.id} className="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm border border-gray-100"><div className="flex items-center gap-3"><div className={`p-2.5 rounded-xl ${t.type === 'income' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>{t.type === 'income' ? <TrendingUp size={18}/> : <TrendingDown size={18}/>}</div><div><p className="font-bold text-sm text-gray-800 tracking-tight">{t.title}</p><p className="text-[10px] text-gray-400 font-bold uppercase">{t.date}</p></div></div><p className={`font-black text-sm ${t.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>{t.type === 'income' ? '+' : '-'}{t.amount.toLocaleString()}</p></div>
              ))}
            </div>
          </div>
        )}
